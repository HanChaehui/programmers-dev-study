...
- 싱글톤 빈, 서비스 오브젝트는 **변경 가능한 요청별 상태**를 가지지 않는 **무상태 stateless 객체**로 만들어야 한다.
- 요청마다 달라지는 데이터는 필드 대신 **메서드의 지역 변수나 파라미터로 처리하는 것이 안전**하다. (지역 변수는 스레드마다 따로 생성)

위험 값 :  특정 요청의 사용자 정보, 처리 중간값 등
(예: String currentUserId, User loginUser, int temporaryCount)

가능 값 : 읽기 전용 값, 다른 싱글톤에 대한 참조
(예: UserDAO가 ConnectionMaker를 필드로 가짐)

## 싱글톤 스코프 vs 요청 스코프

- 싱글톤: 스프링 컨테이너 안에서 해당 빈을 하나만 만들어 공유 
→ 공유 객체이므로 변경 가능한 요청별 상태를 가지면 안 됨 (무상태 원칙).
- 요청 스코프: 새로운 HTTP 요청이 생길 때마다 새로운 독립된 빈 생성
→ 그 요청 동안의 상태를 안전하게 담아도 됨 (다른 요청과 안 섞임).

#### 요청 스코프 빈 - HTTP 요청이 몰리면 대기하는가?

- 대기 여부는 빈 스코프가 아니라 **WAS 스레드 풀 크기(스레드 수)가 결정**한다.
- 기본 싱글톤 스코프와 다르게 웹을 통해 새로운 HTTP 요청이 생길 때마다 빈을 새로 생성한다.
- 대기가 생긴다면, 요청 스코프 빈이 병목이라서 대기하는 것이 아니다.
- 요청을 처리할 WAS 스레드가 부족하면 요청 자체가 대기한다.
- 스레드를 배정받아 요청 처리가 시작되고, 그 과정에서 요청 스코프 빈이 필요해지면 해당 요청 전용 빈이 생성된다.

| 구분 | 무엇이 정하나 |
| --- | --- |
| 빈이 새로 만들어지는가 | 스코프가 결정 (request면 요청마다 새로) |
| 요청이 대기하는가 | WAS의 스레드 풀 크기와 요청 큐가 결정 |
...
- private 생성자라서 상속이 불가능하다. 
→ 객체지향의 장점인 상속/다형성/확장 이용 불가
- 테스트가 어렵다. 
→ 객체 생성 방식이 고정되어 가짜(mock)/테스트용 객체로 갈아끼우기 힘들다.
- 의존성 주입이 어색하다. 
→ 두번째 호출부터는 이미 instance가 존재하므로 넘긴 인자인 의존성 객체(simpleConnectionMaker)의 구현체가 무시된다.
...
- static 필드라 사실상 전역 상태가 되어 아무 데서나 접근 가능해진다.
    ...
    → 객체가 어디서 만들어졌고, 누가 쓰고 있고, 어떤 의존성을 가지고 있는지 추적하기 어려워짐, 코드가 전역 상태에 의존하게 돼서 유지보수와 테스트가 어려워짐
    

→ 개발자가 직접 싱글톤 패턴을 구현하면 객체지향적으로 불편한 점이 많으니까, 스프링 컨테이너가 대신 싱글톤으로 관리해준다 (싱글톤 레지스트리)

| 구분 | 직접 만든 싱글톤 | 스프링 싱글톤 |
| --- | --- | --- |
| 객체 하나만 생성 | 직접 static으로 관리 | 컨테이너가 관리 |
| 생성자 | private으로 막음 | 일반 public 생성자 가능 |
| DI | 어색함 | 자연스럽게 가능 |
| 테스트 | 어려움 | 상대적으로 쉬움 |
| 상속/다형성 | 제한됨 | 가능 |
| 관리 책임 | 클래스 자신 | 스프링 컨테이너 |