...
#### `UserDAO`와 실제로 관계 맺을 DB 연결 오브젝트 타입을 인터페이스 타입으로 추상화
...
- `UserDAO`는 실제로 관계를 맺을 오브젝트의 타입을 몰라도 됨
- 클라이언트 코드( `UserDAO` )에서 생성자의 파라미터 등으로 **런타임 사용 관계**(런타임 링크, 런타임 의존관계)를 맺어줌
...
변경이 일어날 때 모듈의 많은 부분이 함께 바뀐다면 응집도가 높다고 말할 수 있다. 만약 모듈의 일부분에만 변경이 일어나도 된다면, 모듈 전체에서 어떤 부분이 바뀌어야하는지 파악해야 하고, 또 그 변경으로 인해 바뀌지 않는 부분에는 다른 영향을 미치지 않는지 확인해야하는 이중의 부담이 생긴다.
